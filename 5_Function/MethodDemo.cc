...
// 演示二义性调用
void manip(long l){
    cout << "调用了long类型函数" << endl;
}
void manip(float f){
    cout << "调用了float类型函数" << endl;
}
...
int main(int argc, char const *argv[]) {
    ...
    */
    ...
    /*
        ...
        1 确定候选函数
            函数匹配的第一步就是选定调用对应的重载函数集, 集合中的函数称为候选函数. 候选函数具备两个特征:
            * 于被调用的函数同名
            * 声明在调用点可见
        2 确定可行函数
            函数匹配的第二步就是考察本次调用的实参, 然后从候选函数中选出能被这组实参调用的函数, 这些新选出来的函数
            称为可行函数,  可行函数的两个特征
            * 其参数数量与本次调用的实参数量相等
            * 每个实参的类型与对应的形参类型相同, 或者可以转换成形参的类型
        3 寻找最佳匹配
            函数匹配的第三部就是从可行函数中选择与本次调用最匹配的函数. 最基本的思想就是, 实参类型与形参类型越接近, 他们匹配的越好.

            关于最佳匹配, 编译器将实参类型到形参类型的转换划分了几个等级
            1. 精确匹配, 包括, 实参形参类型相同, 实参从数组类型或函数类型转换成对应指针类型, 对实参进行顶层const的增删
            2. 通过const转换实现的匹配
            3. 通过类型提升实现的匹配
            4. 通过算术类型匹配
            5. 通过类类型转换实现的匹配
    */
    //manip(2.14);   // 由于2.14是double类型, 它既能转换long也能转换成float. 因为存在两种可能的算术类型转换, 所以该调用具有二义性
